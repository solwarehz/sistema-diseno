...
export const CAMBIOS = [
  {
    ...
    porque:
      ...
      + 'por que enterarse de esto para dejar de estar roto. Para esa forma —y SOLO para ella— el '
      + 'gancho mantiene un `MutationObserver` sobre el documento MIENTRAS VIVE, no una sola vez, y '
      + 'en desarrollo avisa. Hace falta mirar el documento porque un `RefObject` no avisa cuando se '
      + 'llena. Se intento sustituirlo por «reconciliar despues de cada render» y el razonamiento '
      + 'parecia impecable y era falso: el efecto corre despues de cada render DEL COMPONENTE QUE '
      + 'LLAMA AL GANCHO, no del arbol, asi que con la caja montada por otro —un `Suspense`, un hijo '
      + '`memo`— nadie reconcilia nada. Medido: la caja existe, cero observadores, capacidad 1x1. Lo '
      + 'encontro una SEGUNDA auditoria despues de darlo por cerrado, y las pruebas no lo veian '
      + 'porque todas cambiaban el estado en el mismo componente del gancho: un fixture comodo es un '
      + 'fixture que miente. '
      ...
      + 'nodo? Con el codigo de la v1.147.0 devuelto a su sitio caen ONCE pruebas — medido, no '
      + 'estimado; esta entrada dijo «dos», luego «seis», y las dos eran cifras sin contar. Se deja escrito el recorrido en vez de borrarlo: es el mismo defecto que se le reporta al equipo, en pequeño, y la de «seis» viajo ya en un commit. '
      + 'Y LA PRIMERA VERSION DE ESTE ARREGLO TAMPOCO ESTABA BIEN. Salio en verde con 39 pruebas y una auditoria la tumbo por tres sitios: en `StrictMode` React vuelve a correr el efecto pero NO vuelve a llamar la retrollamada del `ref`, asi que anular el nodo en la limpieza dejaba sordo justo al camino recomendado —y el equipo usa modo estricto—; si la caja se desmontaba y volvia se seguia midiendo el nodo viejo, ya desprendido, que mide cero; y al pasar `medir` a leer solo el nodo propio se PERDIO el rescate por `resize` que la v1.147.0 si tenia, que es una regresion. Los tres son el mismo error: tratar el enganche como un SUCESO que ocurrio una vez en vez de como un ESTADO que hay que mantener. Ahora el gancho se reconcilia despues de cada render —una comparacion de referencias, sin maquetado forzado— y el vigilante del documento queda SOLO para la forma con `RefObject`, que es la unica que lo necesita. Y lo que devuelve se memoriza: envolver la capacidad para anadirle el `ref` hacia que cambiara de identidad en cada render, y eso dispara los efectos del consumidor que dependan de ella. '
      + 'Y MEDIR TODAS LAS PANTALLAS TRAJO CINCO DEFECTOS QUE NADIE BUSCABA. (1) `.sr-solo` es `position: absolute` con desplazamientos `auto` y padre estatico, asi que su bloque contenedor queda FUERA de los contenedores que desplazan y el recorte no le alcanza: medido, la pagina entera se desplazaba 554 px en horizontal a 1024 —scrollWidth 1579 contra 1024— por el texto que solo ve un lector. Ningun elemento «se salia»: todos estaban dentro de algo que recorta y el recorte no les llegaba. La hoja que viaja declara desplazamiento en QUINCE selectores; dos ya eran bloque contenedor —`.sel-lista` y `.app-lista`— y a los TRECE restantes se les anade `position: relative`: una linea cada uno y ningun cambio visual. Dos mas, solo del catalogo, van con ellos. Y una prueba barre la hoja entera regla a regla, para que no vuelva a entrar uno sin el. Se verifico que NO recorta el panel flotante de la lateral plegada: se sigue pintando 20 px mas alla del borde del carril, igual que antes. '
      + '(2) La demo de PADRE HOSTIL era marcado escrito a mano —seis celdas fijas en una caja fija—, asi que a 360 px la caja daba para 2x2 y las seis seguian ahi: la demostracion de la regla 16 incumplia la regla 16. Ahora monta el gancho. '
      + '(3) La barra del catalogo se salia 22 px a 768 porque envolvia solo bajo 700 px. Envuelve siempre: un umbral elegido a ojo acierta en el ancho en que se miro y falla en el siguiente. '
      + '(4) EL INDICE DE PARADA SE DESVIO 301 PX, Y LO CAUSO EL ARREGLO (1). Al hacer el carril '
      + 'bloque contenedor, el carril paso a ser el `offsetParent` de sus paradas, y la cuenta del '
      + 'indice restaba ademas el sitio del carril: lo que antes compensaba se volvio sesgo puro. '
      + 'Medido en el catalogo: 301 px sobre un paso de 543 —mas de MEDIA PAGINA—, asi que EN '
      + 'REPOSO en la pagina 1 la tira decia «Pantalla 2 de 4», dentro de un `aria-live` que lo '
      + 'anuncia. La regla 17(b) decia que `offsetLeft` «no se puede equivocar»; si puede, y ya no '
      + 'lo dice. Un arreglo puede abrir un defecto en otro sitio. '
      + '(5) EL AVISO DE CAJA CORTA CALLABA EN DIEZ PIXELES. Su umbral eran 114 —la celda— y la rejilla reserva 10 por arriba, asi que una fila pide 124. Lo encontro el propio catalogo con el flujo en vivo corriendo: caja de 121, fila de 124, 3 px fuera y el carril desplazandose en vertical —lo que el sistema promete que no pasa— en silencio. Un umbral que ignora la reserva que la propia cuenta descuenta miente justo en el margen donde hace falta. '
      + 'Y EL TABLERO CON LOS DATOS CAMBIANDO DEBAJO, que lo pidio el responsable. El sistema no trae socket.io —el transporte es del producto—; lo que garantiza es que la pieza no mienta cuando la lista se mueve. El catalogo monta un flujo simulado que completa el turno y vuelve a empezar, que es cuando la lista ENCOGE de 24 a 18 y el carril podria quedarse senalando una parada que ya no existe. Medido en vivo: las paradas bajan de 5 a 4 y la tira las sigue. Salieron tres cosas mas: la clave de celda tiene que ser el nombre y no el indice —con el indice la foto de una persona se queda puesta en otra—, el rotulo tiene que decir la hora REAL del ultimo dato y no una escrita a mano, y el reloj no puede retroceder.',
  ...
  },
  {
    ...
    porque:
      ...
      + 'Y EL PUBLICADOR DABA VERDE EN FALSO POR TERCERA VEZ, con un disfraz nuevo: un worker de '
      + 'vitest murio por memoria a mitad del barrido y vitest imprimio «Test Files 58 passed (59)», '
      + '«Tests 1198 passed (1213)» y salio con CODIGO 0. El paso mira el codigo de salida —lo '
      + 'aprendido el 2026-09-11— y habria dicho «1198 en verde» con un archivo entero SIN CORRER. '
      + 'Ahora compara los archivos que corrieron con los que vitest se propuso correr: lo que no '
      + 'corrio no protege nada, y una comprobacion que mide lo que paso sin mirar lo que FALTABA es '
      + 'la misma ceguera de la lista de candados incompleta. Y al mirar QUE archivo faltaba salio '
      + 'la causa: `marco-catalogo.test.tsx` abria un JSDOM del catalogo entero —2,8 MB— en CADA una '
      + 'de sus siete llamadas y no cerraba ninguna. Es el mismo agotamiento que ya estaba '
      + 'documentado y arreglado en ese mismo archivo para las ventanas que SI ejecutan guiones; la '
      + 'mitad que solo lee el marcado se quedo fuera del arreglo. Con un unico arbol compartido, de '
      + '58 s a 18 s.',
    tokens: { alta: [], baja: [] },
    rompe: [
  ...
  },
  {
    ...
    porque:
      ...
      + 'y decide el orden. '
      + 'OCHO REGLAS NUEVAS DE CONTRATO —18 a 25— CON 54 PRUEBAS, y las tres ultimas las escribieron '
      + 'DOS AUDITORIAS ADVERSARIAS antes de publicar. Lo que encontraron es lo que mas sirve contar. '
      + 'UNO · CON `filas` Y EL `base` POR OMISION, EL MODULO ENTERO VIAJABA VACIO. `base` se buscaba '
      + 'por id literal, y con varios recursos por modulo los ids son unicos —`trab-ver`, `cont-ver`— '
      + 'asi que NUNCA existia uno llamado `ver`: `privilegiosEfectivos` devolvia `{}` pasara lo que '
      + 'pasara con los interruptores, y con un backend de juego completo eso BORRA permisos que nadie '
      + 'retiro. Es el daño exacto que el R150 cerro, reabierto por la puerta de al lado. Nadie lo veia '
      + 'porque las cuatro invocaciones de matriz que existian —tres pruebas y el catalogo— pasaban '
      + '`base={null}`: EL CAMINO POR OMISION NO LO EJERCIA NADIE. Ahora `base` nombra la COLUMNA base '
      + 'y manda sobre SU fila. '
      + 'DOS · «ES EL MISMO CODIGO» ERA CIERTO AL PIE DE LA LETRA Y FALSO EN SU PROPOSITO. La matriz '
      + 'consumia cuatro de los seis campos que el calculo devuelve y tiraba `conQuien` (R99) y '
      + '`arrastraElBase` (R149), y no pasaba `ayuda` (R148) a la celda: tres reglas ya escritas que la '
      + 'lista cumplia y la matriz no. No eran dos logicas — era una logica y DOS PANTALLAS DICIENDO '
      + 'COSAS DISTINTAS sobre quien puede que, que es justo lo que se queria evitar. Y de paso se vio '
      + 'que `Interruptor` NUNCA ato su `ayuda` al control: un lector leia «Editar, interruptor, '
      + 'desactivado» y se dejaba el porque, en la lista tambien, desde que R148 la puso ahi. '
      + 'TRES · CINCO FORMAS DE PERDER UN PRIVILEGIO SIN UN SOLO AVISO —sin `columna`, con una '
      + '`columna` o una `fila` que no existen, dos en la misma celda, o `fila` sin `filas`— y en dos '
      + 'de ellas EL PERMISO SEGUIA CONCEDIDO y viajando al backend: activo, y ausente de la pantalla '
      + 'donde se revisa. El componente gritaba cuando faltaban `columnas` y callaba cuando se evaporaba '
      + 'un permiso. '
      + 'CUATRO · «QUINCE MUTACIONES, QUINCE EN ROJO» ERA CIERTO Y NO ERA COBERTURA. Sobrevivian 23, y '
      + 'la peor dejaba la matriz SIN PODER RETIRAR UN PERMISO: ninguna de las trece pruebas pulsaba un '
      + 'interruptor ENCENDIDO. Se paso de 13 pruebas a 54 y se volvieron a mutar una a una. '
      + 'Y CIFRAS: la memoria decia «1071 pruebas en 54 archivos» con 1117 en 55, «177 unidades» con '
      + '178 y «60 trazos» con 62; y el catalogo decia «el contrato tiene diecisiete» con veinticinco '
      + 'escritas, en el mismo parrafo que advierte que «una lista cerrada con numero en el titulo es '
      + 'una lista que envejece». AHORA ESA CIFRA SE CUENTA DEL CONTRATO. El panel estrena ademas su '
      + 'bloque de «copia esto», que no tenia ninguno.',
    tokens: { alta: [], baja: [] },
    rompe: [
  ...
  },
  {
    ...
    porque:
      ...
      + 'Y LO QUE ENCONTRARON LAS DOS AUDITORIAS Y EL NAVEGADOR. '
      + 'MIRANDOLO EN CHROME: la N.o anclada salia a 45,24 px con 26 columnas —en una tabla mas '
      + 'ancha que su contenedor, `width` en una celda de tabla ES UNA SUGERENCIA— y quedaba una '
      + 'RENDIJA de 7 px por la que se ve pasar el contenido. Y al reves, con la N.o de cuatro '
      + 'cifras la celda crecia a 55,21 y el nombre se montaba ENCIMA 3,21 px; con cinco, once. '
      + 'Ningun candado podia ver ninguno de los dos: todos miran declaraciones, no cajas. '
      + 'EL SEPARADOR SE PODIA MATAR EN SILENCIO: quitando solo `position: absolute` del `::after` '
      + 'los veinte candados y las once pruebas salian en verde —sin el, el pseudoelemento es una '
      + 'caja EN LINEA y `width: 1px` no aplica—. Lo mismo el `overflow: hidden` del techo movil, '
      + 'que es la propiedad que da efecto a las otras dos, y el `z-index`. '
      + 'Y LA PRUEBA DEL FONDO PASABA POR ACCIDENTE: su ayudante casaba con todo selector que '
      + 'TERMINARA en `.tb-ancla`, asi que se satisfacia con el fondo de otra regla. '
      + 'TODO EL MARCADO DEL R142 QUEDABA FUERA de los candados que comparan catalogo contra '
      + 'entrega —la unica demostracion vive dentro del paquete vivo, que se corta antes de leer—: '
      + 'entran cuatro casos nombrados en verificar-promesa, y se corrige el comentario de vivo.tsx '
      + 'que afirmaba lo contrario. `.tb-f-celda.tb-ancla` PERDIA EL EMPATE contra '
      + '`.tb-fila-filtros .tb-f-celda`: su background no ganaba nunca, que es la declaracion '
      + 'muerta del R87 repetida veinte lineas mas abajo de esa leccion. Y el bloque de «copia '
      + 'esto», el manual y la regla 17ter seguian diciendo `columnasFijas`. '
      + 'EN RangoFecha: la previsualizacion SE RECORTABA AL TECHO y eso era una mentira —con inicio '
      + 'el 1 y tope 7, sobrevolar el 20 pintaba 01→07 y pulsar el 20 no hacia NADA—; ahora un dia '
      + 'fuera de alcance no pinta nada. Y ELIGIENDO EL INICIO NO HABIA PREVISUALIZACION NINGUNA, '
      + 'que con el tope simetrico es un camino de primera clase. Abrir «Desde» con un final lejano '
      + 'dejaba los 61 dias visibles apagados sin una pista: la ventana va ahora al final que hay. '
      + 'El aviso de un periodo rechazado se quedaba pegado tras elegir un rango valido. Un rango '
      + 'INVERTIDO que llega se pintaba sin una palabra. Y el nombre accesible decia «fuera del '
      + 'maximo de 1 dias» y lo decia sobre el dia que uno mismo acababa de poner, sin decir que '
      + 'quien fija el limite es el OTRO extremo.',
    tokens: { alta: [], baja: [] },
    rompe: [
  ...
  },
  {
    ...
    porque:
      ...
      + 'verlo: mide TEXTO SOBRE FONDO y los cuatro lo pasaban a 12,48:1 en claro y 12,19:1 en '
      + 'oscuro, precisamente por ser el '
      ...
      + 'Y LO QUE ENCONTRARON LAS DOS AUDITORIAS ADVERSARIAS, porque es lo que mas sirve contar. '
      + 'EL CANDADO NUEVO SALIA VERDE DELANTE DEL DEFECTO QUE NACIO PARA CAZAR: resolvia el relleno '
      + 'UNA vez y leia los dos modos DEL TOKEN, asi que una regla `[data-tema=oscuro] .chip…` '
      + '—forma que la hoja ya usa en cuatro sitios— devolvia el R143 entero en oscuro sin que nadie '
      + 'lo viera. Ahora cada modo se resuelve con su <html> colgando de la cadena. Y UN TONO NUEVO '
      + 'PODIA ENTRAR SIN VIGILAR: el mapa de Chip.tsx se leia con una expresion suelta, asi que un '
      + 'valor de dos clases se colaba y el candado seguia diciendo «10 nombres son 10 tonos». Ahora '
      + 'se leen las CLAVES y se exige una clase por tono. LA FORMULA NO TENIA NI UNA PRUEBA y su '
      + 'cabecera decia que si —«los datos de Sharma en probar-candado.mjs», que prueba ESLint y '
      + 'nada mas—: es lo que CLAUDE.md §9 llama fatal, cometido aqui. Entran los 32 pares '
      + 'publicados de Sharma, Wu y Dalal a 1e-4, la simetria sobre los 32 y los anclajes; el del '
      + 'blanco salio en rojo y tenia razon —daba L*=100,000004 con croma 0,014 porque el blanco de '
      + 'referencia no se sacaba de la propia matriz—. Y UNA CIFRA COPIADA DEL VECINO EQUIVOCADO: '
      + 'los cuatro chips pasaban el candado de contraste a 12,48:1 en claro y 12,19:1 en oscuro, '
      + 'no a 12,03:1, que es texto-principal sobre error-fondo y es de otro componente. '
      + 'EN RangoFecha: `atajosDeDias` devolvia el `hoy` recibido CON SU HORA y el redondeo subia a '
      + 'partir de las 12:00, asi que «Ultimos 7 dias» DESAPARECIA A MEDIODIA con maxDias={7} y el '
      + 'ejemplo de la documentacion dejaba el panel vacio media jornada. Las pruebas no podian '
      + 'verlo porque el reloj del banco es medianoche exacta. Arreglado en los dos sitios: la '
      + 'medicion cuenta DIAS y el origen sale a medianoche, cada mitad con su prueba. Un `rango` '
      + 'del producto que LANCE ya no tumba el campo al pintar. `atajosDeDias` sanea su argumento '
      + 'como hace maxDias —0 y negativos daban rangos INVERTIDOS que cabian en cualquier tope—. Y '
      + 'el panel de periodos GANA NOMBRE: el id de su rotulo llevaba desde que nacio sin que nadie lo '
      + 'referenciara. La afirmacion R141 sondeaba 400 y 1280 y nada mas, asi que el corte se podia '
      + 'mover de 640 a 401 CON EL CANDADO EN VERDE; ahora se mide en 640, en 641 y tambien el '
      + '`width`. Los dos bloques de «copia esto» de RangoFecha y TablaDatos NO COMPILABAN: '
      + 'corregidos, y queda declarado que ningun candado mira el texto de un bloque de codigo.',
    tokens: { alta: [], baja: [] },
    rompe: [
  ...
  },
  {
    ...
    porque:
      'R119 del registro del equipo, y llevaba parado desde el 09/09 esperando una cosa: '
      + 'saber que formato sobrevive al papel. Ahora el PDF esta escrito y medido, y con eso '
      + 'la peticion se reescribio entera. `AreaTexto` es texto plano; esto es lo que hace '
      + 'falta cuando el texto VIAJA A OTRO FORMATO —un PDF, una impresora, un correo— y por '
      + 'el camino hay un saneador que admite muchisimo menos de lo que un editor de navegador '
      + 'emite. En sus palabras: «lo que el editor ofrece de mas se guarda sin error y '
      + 'DESAPARECE EN EL DESTINO». '
      + 'LA PIEZA NO ES UN EDITOR: ES LA GARANTIA, y cabe en una frase — lo que llega a '
      + '`onCambio` SIEMPRE esta dentro de `etiquetas` y `huecos`, venga de teclear, de pegar, '
      + 'de arrastrar o de deshacer. Si eso se cumple, el mismo componente sirve para un '
      + 'memorandum, para una carta de diez paginas, para el cuerpo de un correo o para una '
      + 'plantilla de SMS. Si no, NO SIRVE PARA NINGUNO: cada producto tendria que volver a '
      + 'sanear lo que el editor le entrega. '
      + 'LAS TRES LISTAS ENTRAN DESDE FUERA —`etiquetas`, `huecos`, `maximo`— y no es '
      + 'configurabilidad por gusto. La lista cambia SIN QUE ESTE SISTEMA PUBLIQUE: ellos '
      + 'retiraron tres huecos un martes porque salian impresos como «[falta jornada]», y si '
      + 'la lista viviera dentro eso habria sido una version nuestra. Cada producto admite '
      + 'cosas distintas: el suyo no tiene cursiva porque su destino embebe DOS fuentes. Y el '
      + 'componente no debe saber que hay al otro lado. '
      + 'LA BARRA SE DIBUJA DESDE LA LISTA, no es un juego fijo con botones apagados, y el '
      + 'motivo es de fondo: una etiqueta que el destino IGNORA es peor que una que rechaza. '
      + 'Rechazada, quien escribe se entera; ignorada, guarda sin error y lo descubre CUANDO YA '
      + 'FIRMO EL PAPEL. El icono `cursiva` existe en este sistema y aun asi no sale si `em` no '
      + 'esta en la lista. '
      + 'CERO ATRIBUTOS, siempre, ni en las etiquetas permitidas. No es la rareza de un '
      + 'producto: es lo que pasa siempre que el HTML se TRADUCE a otro formato en vez de '
      + 'renderizarse. Y se sanea RECORRIENDO EL DOM y no con expresiones sobre el texto: un '
      + 'troceador por texto se engana con `<p title="<strong>">` y con los comentarios '
      + 'condicionales de Word. '
      + 'PEGAR ES LA FUNCION MAS VALIOSA, y lo dijeron ellos: vale mas que cualquier boton de '
      + 'la barra. Es el camino por el que de verdad entra el contenido —Word, Docs, un '
      + 'correo— y se limpia EN EL MOMENTO DE PEGAR, no al guardar, porque al guardar quien '
      + 'escribe pierde lo pegado en el peor momento posible. '
      + 'Y NO SE PINTA LO QUE EL DESTINO NO PUEDE PROMETER: un `h3` sale en negrita y del mismo '
      + 'cuerpo, no a 24px. Este editor ensena ESTRUCTURA, no apariencia final. Pintar una '
      + 'jerarquia que el papel no tiene es la misma mentira que ofrecer una etiqueta que se '
      + 'ignora. '
      + 'SIN LIBRERIA (§9.4 solo la admite para dialogo, menu y selector con busqueda), y aqui '
      + 'ademas seria contraproducente: ninguna libreria conoce la lista del producto, asi que '
      + 'la invariante habria que montarla igual encima. '
      + 'EL SANEADOR VIVE EN SU PROPIO ARCHIVO, y no es estetica: es una funcion PURA sobre '
      + 'texto, asi que se prueba entera sin navegador —incluido lo que pega Word '
      + 'con sus comentarios condicionales—. jsdom no implementa `contenteditable`, ni la '
      + 'seleccion, ni `execCommand`, asi que lo que se prueba a conciencia son las DOS '
      + 'FUNCIONES PURAS —el saneador y la decision de que se lleva un Retroceso, 49 pruebas— '
      + 'y del componente lo que no depende del navegador. «La entrega real» ensena el '
      + 'marcado con la hoja que viaja y nada mas. '
      + 'Y DOS AUDITORIAS ADVERSARIAS LO TUMBARON ENTERO ANTES DE PUBLICAR, con hallazgos que '
      + 'ninguno de los CATORCE candados vio —son catorce candados dentro de diecisiete '
      + 'pasos: los otros tres son generadores—: '
      + '(1) LA IDEMPOTENCIA ERA FALSA. Borrar un hueco desconocido FABRICABA otro juntando '
      + 'las llaves de los lados —`{{ {{documento}}sede}}` daba `{{ sede}}` y despues '
      + '`{{sede}}`—, asi que el documento guardado CAMBIABA SOLO entre una sesion y la '
      + 'siguiente. Y el motivo que la regla daba tambien era falso: asignar `innerHTML` no '
      + 'dispara `input`, asi que el bucle que describia no puede ocurrir. '
      + '(2) EL VALOR QUE ENTRABA NO SE SANEABA. La invariante cubria la salida y la entrada '
      + 'no tenia guarda: un `<img onerror>` guardado por otra version SE EJECUTA al asignar '
      + '`innerHTML`. Una garantia con un solo sentido no es una garantia. '
      + '(3) LA CAJA NO TENIA NOMBRE ACCESIBLE. Un `<label for>` NO nombra a un elemento no '
      + 'etiquetable, y este sistema lo tiene escrito DOS VECES —en `Interruptor` y en '
      + '`RangoFecha`— y aun asi se incumplio. '
      + '(4) «LA FICHA ES INDIVISIBLE» SE AFIRMABA EN TRES SITIOS Y NO HABIA NADA: ni un '
      + '`onKeyDown`, ni un `beforeinput`. Un Retroceso al lado dejaba `{{sede}`. Ahora esta '
      + 'construido, con la decision en una funcion pura para poder probarla. '
      + '(5) LA BARRA DECIA SER UN `toolbar` y no tenia ni roving tabindex ni flechas: eran '
      + 'OCHO tabulaciones antes de llegar al texto. '
      + '(6) LA PAGINA «PRUEBALO» DEL CATALOGO ESTABA MUERTA y REFUTABA la garantia: invitaba '
      + 'a pegar desde Word sobre un `contenteditable` SIN SANEADOR DETRAS. '
      + '(7) El catalogo y el componente no emitian lo mismo en cinco puntos —el icono a 18 '
      + 'contra 16 entre ellos—, y `verificar-iconos` pasaba en verde porque compara nombres '
      + 'y trazo, NO TAMANO. Nace `editor-catalogo.test.tsx`, que compara arbol contra arbol. '
      + 'Y OTRO MAS, este cazado por su propia prueba: el aviso de «esto se retiro al '
      + 'pegar» NO SE VEIA NUNCA. El pegado lo anotaba y llamaba inmediatamente al emisor, que '
      + 'volvia a mirar una caja YA limpia y lo borraba en el mismo cuadro. La funcion entera '
      + 'era decorativa. Cada camino reporta ahora lo que EL retiro. '
      + 'Y (8) EL BOTON PRINCIPAL NO HACIA NADA. Lo midio el equipo en un navegador de '
      + 'verdad: `execCommand("bold")` emite `<b>`, la lista blanca tiene `strong`, y el '
      + 'saneador lo desenvolvia — pulsar Negrita no dejaba negrita. El arreglo es suyo y es '
      + 'pequeño: TRADUCIR despues del comando —`b`→`strong`, `i`→`em`, `strike`/`del`→`s`, '
      + '`ins`→`u`—, NO ampliar la lista blanca, que es el contrato. '
      + 'LA SEGUNDA AUDITORIA, sobre lo ya arreglado, encontro seis mas. El peor NO LO PODIA '
      + 'VER NINGUNA PRUEBA: el saneo llamaba a `DOMParser` DENTRO DEL RENDER y `DOMParser` '
      + 'NO EXISTE EN NODE, asi que cualquier producto con renderizado de servidor —Next, '
      + 'Remix— moria con `ReferenceError` y LA PAGINA ENTERA NO SE PINTABA. Las 787 pruebas '
      + 'corren en jsdom, que si lo tiene. Se cierra con una regla entera y no con un parche: '
      + 'nada que dependa del contenido de la caja se pinta hasta que la caja existe. '
      + 'Y EL AVISO DE LO RETIRADO SE BORRABA SOLO, por dos caminos que no se habian mirado: '
      + 'el `input` que el navegador despacha DESPUES de `execCommand`, y el `blur` de irse a '
      + 'pulsar la barra. EL GESTO NATURAL DE REACCIONAR AL AVISO LO DESTRUIA. Ahora el aviso '
      + 'recuerda DE QUE CONTENIDO habla. Ademas: el borrado de LINEA estaba roto —se metio '
      + 'en el mismo saco que el de caracter y borraba solo la ficha—; la region viva se '
      + 'CALLABA el aviso del tope en cuanto algo se retiraba, que es justo lo que el contador '
      + 'compartido dice haberse extraido para impedir; una prueba decia comprobar el origen '
      + 'del aviso y su unica asercion era otra cosa (se le quito entero el mecanismo que '
      + 'nombra y siguio EN VERDE); y `.ed-mal` y `.ed-retirado` VIAJABAN EN LA HOJA DE TODOS '
      + 'LOS PRODUCTOS sin que el catalogo los pintara en ningun sitio — el aviso que es la '
      + 'unica prueba visible de la promesa central de la pieza se entregaba sin que nadie lo '
      + 'hubiera visto. Ya tiene muestra en «La entrega real», y se compara arbol contra arbol. '
      + 'CIFRAS QUE ESTABAN MAL Y AHORA ESTAN MEDIDAS: el tiempo de pegado se daba como un '
      + 'numero solo —2,3 s para 4,3 MB— y DEPENDE TANTO DEL CONTENIDO QUE UN NUMERO SOLO '
      + 'ENGAÑA: con HTML sucio de Word medimos 3,0 s (3,02 / 2,88 / 3,03) y una auditoria con '
      + 'otro corpus midio 2,7 s. El 2,3 s es anterior y no dejo escrito con que se midio, asi '
      + 'que no se compara. Lo que sostienen las tres medidas, y es lo que importa, es que un '
      + 'documento de ese tamaño bloquea el hilo SEGUNDOS. Todo con jsdom dentro del '
      + 'contenedor, que no es un navegador. '
      + 'TRES HUECOS DECLARADOS: lo que no tiene prueba es el `execCommand` real, NO la '
      + 'costura del rango —esa si la tiene, y la declaracion anterior era mas pesimista que '
      + 'la realidad—; `document.execCommand` esta OBSOLETO en la norma y sigue siendo la '
      + 'unica forma de hacer esto sin libreria; y pegar un documento muy grande bloquea el '
      + 'hilo, con la cifra de arriba, porque `maximo` avisa DESPUES y no impide. '
      + 'DOS ARREGLOS EN LA HOJA QUE TOCAN A QUIEN NO USE EL EDITOR: el buscador de '
      + '`TablaDatos` declaraba `min-width` DOS VECES en el mismo selector —0 y 230px— y '
      + 'ganaba el ultimo, asi que NO BAJABA DE 230px y empujaba la barra de filtros en anchos '
      + 'estrechos, lo contrario de lo que dice su propio comentario; y el editor en error '
      + 'llevaba filete de 1px donde todo campo del sistema lleva 2, que es la señal NO '
      + 'CROMATICA. '
      + 'QUEDA DECLARADO Y SIN CERRAR: ESLint NO SE CORRE en ninguno de los diecisiete pasos '
      + '—solo `probar-candado.mjs`, que prueba los patrones contra casos sinteticos—, y al '
      + 'correrlo sobre el repositorio salen DOS ERRORES QUE VIAJAN HOY: `Estados.tsx` usa el '
      + 'atributo `style` en linea en dos sitios, que es lo que §2.5.6 prohibe. No se toca un '
      + 'componente publicado la vispera de una entrega; el arreglo de fondo es que el paso de '
      + 'lint exista.',
    tokens: { alta: [], baja: [] },
    rompe: false,
  },
  ...
  },
  {
    ...
    porque:
      ...
      + 'ser tapar.'
      + ' Y AL ESCRIBIR ESTA ENTRADA aparecio otro: el REGISTRO DE CAMBIOS se pintaba en '
      + 'el catalogo SIN ESCAPAR, asi que una entrada que mencionara una etiqueta la '
      + 'convertia en etiqueta de verdad. Esta misma, al decir «toda clase que apareciera '
      + 'en un script», abrio un bloque de guion en mitad de la prosa y MATO el guion del '
      + 'catalogo: sin navegacion, sin calendario, sin nada, y el generador terminando con '
      + 'exito. Explicar un defecto obliga a nombrar etiquetas, y nombrarlas no puede romper '
      + 'la pagina que lo explica. Ahora la prueba del calendario exige ademas que el guion '
      + 'del catalogo NO LANCE: se comprueba ejecutandolo, porque un troceador por texto se '
      + 'corta solo en cuanto hay un cierre de guion dentro de un bloque de cargamento.',
    tokens: { alta: [], baja: [] },
    rompe: false,
  },
  {
