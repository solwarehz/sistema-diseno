...
| **21** | **Obligatorio.** (R151, v1.128.0) **La matriz calcula y DICE lo mismo que la lista, porque es el mismo código.** `depende`, `base`, `cerrado`, `deshabilitado`, el punto fijo de `privilegiosEfectivos` y los dos valores de `onCambio` se comportan igual en las dos presentaciones: el estado de cada privilegio se resuelve en un sitio y lo único que cambia es dónde se pinta. Quien lo pidió lo dijo mejor: *«lo que no queremos es reimplementar `depende` y los cuatro estados por nuestra cuenta otra vez»*. **Y «el mismo código» no basta, que es lo que enseñó esta versión**: la primera matriz consumía cuatro de los seis campos que el cálculo devuelve y tiraba `conQuien` (R99) y `arrastraElBase` (R149), y no pasaba `ayuda` (R148) a la celda. No eran dos lógicas — era una lógica y **dos pantallas diciendo cosas distintas sobre quién puede qué**, que es exactamente lo que se quería evitar. Ahora el aviso va en el globito **y** atado al interruptor con `aria-describedby`, porque un `title` no lo anuncia un lector ni lo ve quien navega con teclado. |
...
<!-- pruebas: PanelPrivilegios.test.tsx, privilegios-depende.test.tsx, privilegios-adopcion.test.tsx, privilegios-matriz.test.tsx -->
...
| **5** | **Obligatorio.** (R97, v1.72.0) **Sin el privilegio base, el resto SE MARCA pero NO se oculta.** Siguen ahí, visibles y **pulsables**, con un aviso que lo explica. Hacerlos desaparecer haría pensar que se perdió lo que estaba concedido. *(Decía «se atenúa» y «apagados», y desde la regla 11 las dos palabras son falsas: la atenuación se fue —costaba 2,07:1— y los controles nunca dejaron de pulsarse. Una contradicción viva entre dos reglas del mismo documento la cazó una auditoría.)* |
...
| **11** | **Obligatorio.** (R144, v1.125.0) **Sin el `base`, el resto del módulo se marca con un CARRIL, no con opacidad.** Aquí había `opacity: .5`, y el propio sistema lo llevaba declarado abierto desde la v1.91.0: el nombre caía a **2,07:1** y el icono a **2,03:1** con los interruptores **aún pulsables**, así que la excepción de WCAG para controles inactivos no aplicaba. Lo reclamó Control Administrativos V2.0 con el dato que lo vuelve urgente: **«sin base» no es un caso raro, es el estado INICIAL de la pantalla** — repartir a un cargo nuevo empieza con los diez módulos sin su «ver». La señal era correcta y el canal no: ahora es un filete de 3 px en `aviso-acento`, el mismo del aviso, y **ninguna relación de contraste se toca**: medido en Chrome sobre el catálogo, el nombre pasa de **2,07:1 a 14,33:1** —`texto-principal` sobre `fondo-tarjeta`, sin nada multiplicado por 0,5— y el filete da **4,81:1** contra la tarjeta. No se ocultan —desaparecer un permiso concedido haría pensar que se perdió— y **no se bloquean**: encender algo antes de conceder el base es legítimo, y bloquearlo le quitaría a quien reparte el orden en que quiere trabajar. |
| **12** | **Obligatorio.** (R144, v1.125.0) **El aviso va justo debajo del `base`**, no al final del módulo. Estaba después de las seis filas que describe, así que se leían seis permisos antes de enterarse de que ninguno se aplica. Se **intercala** conservando el orden de la lista: cuál va primero lo decide quien declara los privilegios. |
| **13** | **Obligatorio.** (R145, v1.125.0) **En el teléfono la cabecera conserva QUÉ, no solo CUÁNTOS.** Bajo 900 px los chips se ocultaban y sobrevivía el recuento, y el comentario que lo justificaba —«el conteo dice lo mismo en una línea»— **era falso**: «4 de 6» dice cuántos, no cuáles. El acordeón plegado **es** la vista de repaso, así que en móvil había que abrir los diez módulos uno por uno para responder «¿qué puede hacer este cargo?» — justo el trabajo que el plegado venía a ahorrar. Ahora bajan a una **segunda línea** con los dos primeros y un **«+N»**. El resumen lo emite el componente **siempre** y lo enseña solo la hoja: un componente no puede preguntar cuánto mide la pantalla sin medir, y medir para decidir marcado es lo que hace que el servidor y el navegador pinten cosas distintas. |
| **14** | **Obligatorio.** (R146, v1.125.0) **El bloqueado por `depende` SÍ se alcanza con teclado; los otros tres no.** La v1.91.0 dejó escrito que los cuatro salían del orden de tabulación y que era «defendible en tres, discutible en el cuarto»; Control Administrativos V2.0 trajo el dato que lo decide: **éste es el único transitorio** — se desbloquea sin recargar en cuanto se enciende el otro, así que quien navega con teclado veía aparecer en el recorrido una fila que un segundo antes no podía alcanzar, sin nada que lo anunciara, y no tenía forma de llegar a ella para saber qué le falta. Se emite como `role="switch"` con **`aria-disabled` y no `disabled`** —la misma decisión que el calendario tomó en el R139: apagado de verdad vuelve a salir del recorrido— con su nombre en `aria-labelledby`, su motivo en `aria-describedby` y su anillo de foco. `cerrado`, `ajeno` y `pendiente` son estados **estables y son texto**: quedarse fuera del recorrido es correcto ahí. |
| **15** | **Obligatorio.** (R148, v1.126.0) **`deshabilitado` por privilegio: apaga el control y NO esconde el dato.** Lo único que había por fila era `cerrado`, y `cerrado` **sustituye el interruptor por un chip**: un cargo con el permiso **concedido** se veía como si no lo tuviera. Control Administrativos V2.0 lo trajo con el caso que lo explica: su servidor manda qué puede conceder **quien está mirando**, y que el cargo tenga «Editar» y que tú no puedas tocarlo son **dos hechos distintos y los dos importan**. Es `soloLectura` pero por fila, **sigue contando** en el «4 de 6» y en los chips, y se apaga con **`aria-disabled`, no `disabled`** —lo resuelve `Interruptor`— así que se alcanza con teclado y su estado se lee. El porqué va en `ayuda`: no se inventa un campo de motivo. |
| **16** | **Obligatorio.** (R149, v1.126.0) **El `base` se ANUNCIA antes de pulsar.** Encender cualquier privilegio encendía también el base del módulo y eso no se decía en ninguna parte. El daño no es «conceder de más»: con un **PUT de juego completo** y una regla anti-escalada, un `ver` añadido de rebote que el repartidor no posee hace que el servidor **rechace el guardado entero** — se pierde también el interruptor que la persona sí quería cambiar. **Un efecto invisible convertía una acción legítima en un error.** Se dice en la etiqueta, que es exactamente lo que `clave` ya hacía con su «va con X y Y»: el patrón existía y a esto no se le había aplicado. **Solo cuando va a pasar**: si el base ya está concedido, si éste es el base, si ya está encendido o si el base está `cerrado`, no se dice — una frase que aparece siempre deja de leerse. |
| **17** | **Obligatorio.** (R150, v1.126.0) **Lo efectivo lo entrega el panel, y `privilegiosEfectivos` exige su `base`.** El tercer parámetro era `= 'ver'` y **no seguía al del componente**: montar el panel con `base={null}` y llamar `privilegiosEfectivos(modulos, valor)` **vaciaba todos los módulos sin `ver`**, con las dos llamadas válidas y sin que tipos ni contrato avisaran. Ahora `onCambio` entrega **dos** cosas —el mapa completo y lo efectivo, calculado con el `base` que el panel tiene— y el parámetro deja de tener valor por omisión: la combinación pasa a ser **un error de compilación**. ⚠️ Y se dice en voz alta lo que faltaba: **lo efectivo puede ser un BORRADO.** Un módulo sin su base sale como `{}`, y con un PUT de juego completo eso no significa «no se aplica», significa **borrar sus filas**. |
| **18** | **Obligatorio.** (R151, v1.128.0) **El panel también se presenta como MATRIZ, y es una tabla de verdad.** Con `presentacion="matriz"` y `columnas`, las filas son recursos y las columnas acciones — el patrón estándar de una pantalla de permisos, y el que la lista no puede dar: la lista responde «¿qué puede hacer este cargo con Contratos?» y solo la matriz responde «¿quién puede editar?», que es la revisión que se hace leyendo **hacia abajo**. Se emite `<table>` con `<th scope="col">` y `<th scope="row">`, no una rejilla de `<div>`: doscientos interruptores sin encabezados asociados son una pantalla que solo se puede usar mirándola. **Es el mismo componente, no un hermano**: `PanelPrivilegios` y `MatrizPrivilegios` habrían sido dos verdades sobre quién puede qué, y la primera vez que las reglas 1-17 cambiaran solo cambiaría una. Sin `columnas` no hay matriz que dibujar: se cae a la lista y **se avisa por consola** en vez de pintar una tabla de una columna. |
| **19** | **Obligatorio.** (R151, v1.128.0) **Cada interruptor de la matriz se llama por su CRUCE, no por su columna.** `Interruptor` gana `etiquetaOculta`: la etiqueta existe, va en la fila del lector —«Contratos · Editar»— y no se dibuja. Sin esto, el encabezado se ve pero no viaja al control, y un lector de pantalla recorre la tabla anunciando doscientas veces «Editar». |
| **20** | **Obligatorio.** (R151, v1.128.0) **`noAplica` es el CUARTO motivo, y el hueco vacío no es ninguno.** Omitir el privilegio no es lo mismo que decir que no aplica: omitiendo, quien reparte nunca se entera de que esa acción existe para otros recursos y sí pregunta por qué le falta. Lleva motivo **obligatorio**, porque un «no aplica» sin explicación es exactamente la casilla vacía que venía a sustituir. Una celda sin privilegio declarado se distingue de las cuatro y **no dice nada**. Y no se concede: `privilegiosEfectivos` lo limpia como a los otros tres —contar una acción que no existe haría que un cargo pareciera incompleto por algo que no depende de nadie. |
| **21** | **Obligatorio.** (R151, v1.128.0) **La matriz calcula con la MISMA lógica que la lista, no con una suya.** `depende`, `base`, `cerrado`, `deshabilitado`, el punto fijo de `privilegiosEfectivos` y los dos valores de `onCambio` se comportan igual en las dos presentaciones porque **son el mismo código**: el estado de cada privilegio se resuelve en un sitio y lo único que cambia entre lista y matriz es dónde se pinta. Quien lo pidió lo dijo mejor: *«lo que no queremos es reimplementar `depende` y los cuatro estados por nuestra cuenta otra vez»*. |
| **22** | **Obligatorio.** (R151, v1.128.0) **La columna de nombres se queda quieta al deslizar.** Una matriz es más ancha que su contenedor casi siempre, y sin la primera columna anclada se marca la casilla de la fila equivocada. Es el patrón del R142 —fondo propio, porque una celda pegajosa transparente deja ver pasar el texto de debajo; **suelo y techo**, porque en una tabla más ancha que su caja `width` es una sugerencia y ahí se midió una rendija de 7 px en Chrome; y el hover **después** del rayado, que empatan en especificidad y decide el orden. |
| **23** | **Obligatorio.** (R151, v1.128.0) **Con `filas`, el `base` gobierna SU recurso, no el módulo.** Hasta el R151 un módulo era un recurso y `base` se buscaba por `id` literal. Con varios recursos por módulo los `id` son únicos (`trab-ver`, `cont-ver`), así que **nunca existe uno llamado `ver`** —el valor por omisión— y `privilegiosEfectivos` devolvía `{}` para el módulo entero pasara lo que pasara con los interruptores: con un backend de juego completo, **eso borra permisos que nadie retiró**, el daño exacto que R150 cerró. Ahora `base` nombra **la columna** base y el que manda sobre cada privilegio es el de **su propia fila** (`baseDe`), un recurso sin su «ver» no se lleva por delante a los demás, y el carril de «sin base» marca la fila que le falta y no todas. Lo cazó una auditoría adversaria antes de publicar: las cuatro invocaciones de matriz que existían pasaban `base={null}`, así que **nadie ejercía el camino por omisión**. |
| **24** | **Obligatorio.** (R151, v1.128.0) **Nada desaparece de la matriz sin decirlo.** El componente gritaba cuando faltaban `columnas` y callaba cuando se evaporaba un permiso: hay cinco formas de que un privilegio no caiga en ninguna celda —sin `columna`, con una `columna` o una `fila` que no existen, dos en la misma celda, o `fila` en un módulo sin `filas`— y en dos de ellas **el permiso seguía concedido y viajando al backend**: activo, y ausente de la pantalla donde se revisa. Para una matriz que existe *para* la revisión periódica, esa es la peor salida posible. Ahora cada caso se dice por consola en desarrollo, con el `id` concreto, y también lo que **no cabe** en un cruce: `niveles`, el título de un `grupo`, y `abiertos`/`onAbiertos`. |
| **25** | **Obligatorio.** (R151, v1.128.0) **La matriz se puede recorrer sin ver y sin ratón.** Cada celda declara `headers` con sus dos encabezados —sin eso los `id` de los `th` no los referenciaba nadie y eran atributos muertos—, la celda bloqueada por `depende` conserva su parada de teclado como en la lista (R146, la única transitoria), y con **varios módulos** cada bloque de filas dice de qué módulo es: sin eso, las filas de dos módulos salen seguidas y sin distinguir, y una pantalla de revisión que no dice de qué es cada recurso no se puede revisar. |
| **26** | **Obligatorio.** (R151, v1.129.0) **`base` es un id o una COLUMNA, y se decide por cuál de las dos existe.** No por si el módulo declara `filas`, ni por la presentación: `privilegiosEfectivos` no sabe cuál se está pintando, así que el criterio tiene que vivir en los datos. Si hay una columna que se llama como el `base`, manda el privilegio de **esa columna en la fila de cada uno**; si no, manda el privilegio con ese `id`; y si no existe de ninguna de las dos formas, **no hay base que aplicar** y el módulo no concede nada — que es lo que «sin su base» ha significado desde el R97. La versión anterior preguntaba «¿hay privilegios con `columna`?», y con eso un módulo de ids `ver`/`editar` y columnas `consultar`/`modificar` **se vaciaba entero, también en lista**, donde antes del R151 funcionaba. Basta declarar `columna` una vez para pintar las dos presentaciones, que es lo que las reglas 18 y 21 invitan a hacer. |
| **27** | **Obligatorio.** (R151, v1.129.0) **Un solo punto fijo para las tres razones por las que algo deja de aplicarse.** `cerrado`, un `depende` sin resolver y la falta de base se preguntan en la misma vuelta, y se repite hasta que el mapa deja de encoger. Había **dos bucles separados que no se realimentaban** —y el comentario del segundo decía, con estas palabras, «va dentro del punto fijo y no después», estando después—: con el base de un recurso cerrado, lo que dependía de un privilegio caído **sobrevivía**, porque su turno ya había pasado. Dos permisos viajaban al backend sin su dependencia, que es el 403 del R110 por la puerta de al lado. Y **lo que ya no es un privilegio declarado tampoco se aplica**: una clave guardada de un permiso retirado viajaba concedida, sin interruptor, sin chip y sin contar — se dispara al migrar de lista a matriz, que renombra los ids. El mapa completo las conserva, que es R98; el efecto no. |
| **28** | **Obligatorio.** (R151, v1.129.0) **En la matriz, una fila que no concede nada lo dice con palabras.** El filete de color era su único portador —SC 1.4.1— mientras la lista lleva su frase desde el R144; ahora va dentro del encabezado de fila, que es lo que un lector anuncia **antes** de cada celda. Y **con varios módulos, el nombre del módulo entra en `headers`**: como `headers` sustituye a la asociación por `scope`, dos sedes con una fila «Contratos» cada una daban cuatro interruptores con **dos pares de nombres accesibles idénticos** — la regla 25 se cumplía mirando la pantalla y no sin verla, que es justo lo que dice cubrir. |
