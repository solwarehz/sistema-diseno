...
| **10** | **Obligatorio.** (R102, v1.77.0) **`presentacion` decide la forma, y `caja` sigue siendo el defecto.** `caja` pinta la vista previa a tamaño real —el círculo del avatar, el hueco de 212×44 del logo— y es lo correcto en una pantalla dedicada a poner esa imagen: enseña el hueco real donde va a vivir. `fila` la mete en la fila común de las tres cargas, que es lo que se pide **en un formulario**, donde la caja de 96 px rompía la rejilla. No se cambió el defecto a `fila` a propósito: reformaría en silencio cada selector de foto y de logo ya en producción. En `fila` no hay avatar de reserva — a 22 px unas iniciales no se leen. |
